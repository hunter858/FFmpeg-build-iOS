...
FRAMEWORK="$FRAMEWORK_NAME$FRAMEWORK_EXT"                        #FFmpeg.framework
BUILD_FOLDER="$CURRENT_FOLDER/FFmpeg-iOS"                        #当前路径+/FFmpeg-iOS
BUILD_THIN_FOLDER="$CURRENT_FOLDER/thin"                         #当前路径+/thin
BUILD_INCLUDE_FOLDER="$BUILD_FOLDER/include"                     #当前路径+/FFmpeg-iOS/include
BUILD_LIB_FOLDER="$BUILD_FOLDER/lib"                             #当前路径+/FFmpeg-iOS/lib
OUTPUT_FOLDER="$CURRENT_FOLDER/$FRAMEWORK"                       #当前路径+/FFmpeg.framework
OUTPUT_INFO_PLIST_FILE="$OUTPUT_FOLDER/Info.plist"               #当前路径+/FFmpeg.framework
OUTPUT_HEADER_FOLDER="$OUTPUT_FOLDER/Headers"                    #当前路径+/FFmpeg-iOS/lib/Headers
OUTPUT_UMBRELLA_HEADER="$OUTPUT_HEADER_FOLDER/ffmpeg.h"          #当前路径+/FFmpeg-iOS/lib/headers/ffmpeg.h
OUTPUT_MODULES_FOLDER="$OUTPUT_FOLDER/Modules"                   #当前路径+/FFmpeg.framework/Modules
OUTPUT_MODULES_FILE="$OUTPUT_MODULES_FOLDER/module.modulemap"    #当前路径+/FFmpeg.framework/Modules/module.modulemap
VERSION_NEW_NAME="Version.h"                                     #Version.h
BUNDLE_ID="org.ffmpeg.FFmpeg"

function CreateFramework() {
  rm -rf $OUTPUT_FOLDER                                          #强制删除当前路径+/FFmpeg.framework
  mkdir -p $OUTPUT_HEADER_FOLDER $OUTPUT_MODULES_FOLDER          #创建文件夹
}

function CompileSource() {
  ./build-ffmpeg.sh $ARCHS                                        #编译对应ARM
  ./build-ffmpeg.sh lipo                                          #合并
}

function MergeStaticLibrary() {                                   #合并静态库
  local files=""
